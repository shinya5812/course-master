...
7軸スコアリング（CF/SI/JT/PD/BL/SPD/MK）
...
[Phase 3 Step 1 変更: 2026-03-19]
  ...
  [Phase 3 Step 5 変更: 2026-04-03]
    除外した2軸（バックテスト重賞1,544Rで予測精度に寄与しないと確認）:
    - BF: 着順との相関係数 +0.020（逆効果）
    - HP: 着順との相関係数 -0.030（ほぼ無効）
    残り7軸: CF / SI / JT / PD / BL / SPD / MK
    重みテーブルからBF・HPを削除（他軸の重みは変更なし）
...
class CourseMASTERv73:
    ...
    def score_horse(self, horse_row, team_type='I'):
        ...
        # BF: 血統適性（種牡馬 + 母の父）← [Phase 3 Step 5] 除外
        # バックテスト（重賞1,544R）で着順との相関係数 +0.020（逆効果）を確認。
        # 統計値を保持するため build_statistics の処理はそのまま維持する。
        # sire_dist_stats / bms_dist_stats は pkl に引き続き収録する。
        # axes['BF'] は設定しない
        ...
        # HP: ハンディキャップ適応 ← [Phase 3 Step 5] 除外
        # バックテスト（重賞1,544R）で着順との相関係数 -0.030（ほぼ無効）を確認。
        # 斤量×年齢の計算式が実態を反映できていないと判断。
        # axes['HP'] は設定しない
        ...
        # [Phase 3 Step 5: 2026-04-03] BF・HP除外により7軸化
        # 高重み×2: CF・SI・SPD・JT（純粋実力軸）
        # 低重み×0.3: BL・MK（人気・オッズ依存軸）
        # 通常×1: PD
        AXIS_WEIGHTS = {
