...
def edge_label(edge, threshold: float = 0.06) -> str:
    """エッジ値 → 購入判定ラベル（標準閾値+0.06、阪神稍重は+0.07）"""
    if edge is None:
        return '  -  '
    if edge >= threshold:
        return '★購入推奨'
...
def build_umaren_plan(prediction: dict, horse_map: dict,
                      adi: float, tansho_amount: int, edge_ok: bool) -> dict:
    """
    ...
    戻り値:
        ...
        amount_per  : int  – 1点あたり推奨金額（tansho_amount // 2、最低50円）
        note        : str  – 判断理由テキスト
    """
    if not edge_ok:
        return {'eligible': False, 'candidates': [], 'amount_per': 0,
                'note': '馬連対象外（◎エッジ基準未達）'}
    if adi < 50.0:
        return {'eligible': False, 'candidates': [], 'amount_per': 0,
                'note': 'ADI50未満: 単勝◎のみ'}
    ...
    amount_per = max(50, tansho_amount // 2)

    if not top2:
        return {'eligible': True, 'candidates': [], 'amount_per': 0,
                'note': '馬連候補なし（○全員エッジ+0.03未満）'}

    return {'eligible': True, 'candidates': top2, 'amount_per': amount_per, 'note': ''}
...
def print_result(race: dict, prediction: dict, chaos: dict):
    ...
    # ── ◎/○/▲ ＋ エッジ値（テーブル形式） ──
    cond_2  = race.get('track_cond', '良')
    venue_2 = race.get('venue', '')
    tbl_thr = 0.07 if ('阪神' in str(venue_2) and '稍' in str(cond_2)) else 0.06
    dist_2  = int(race.get('distance', 0))
    ...
    for mark in ['◎', '○', '▲']:
        for horse_name, score, win_prob, idx, edge in prediction[mark]:
            ...
            if mark == '◎':
                if is_g2_longdist:
                    v_str = '⚠G2長距離保留'
                elif edge is not None and edge >= tbl_thr and dist_2 < 2200:
                    v_str = '★購入推奨'
                else:
                    v_str = '✗見送り'
            else:
                v_str = ''
            name_t = horse_name[:8]
            print(f'  │{_dp(" "+mark, 5)}│{_dp(" "+str(hno), 6)}│{_dp(" "+name_t, 18)}│{_dp(" "+pop_str, 8)}│{_dp(" "+ods_str, 8)}│{_dp(" "+edge_str, 9)}│{_dp(" "+v_str, 11)}│')
    ...
    # 条件②: エッジ値フィルター（標準+0.06以上 × 距離2200m未満）
    # 阪神稍重は特別ルール: 閾値を+0.07に引き上げ
    ...
    edge_ok    = (honmei_edge is not None) and (honmei_edge >= edge_threshold) and (honmei_dist < 2200) and not is_g2_longdist
    edge_ng_ld = (honmei_edge is not None) and (honmei_edge >= edge_threshold) and (honmei_dist >= 2200) and not is_g2_longdist
    if is_g2_longdist:
        edge_filt_str = '⚠ G2長距離・ベット保留（2201m以上・ROI157%検証済み）'
    elif edge_ok:
        edge_filt_str = '✅ 購入（エッジ{:+.3f}・閾値{}・{}m）'.format(
            honmei_edge, edge_threshold_str, honmei_dist)
    elif edge_ng_ld:
    ...
    if is_g2_longdist:
        ...
        verdict = f'★  ◎{honmei_name} 単勝 購入推奨（エッジ{edge_threshold_str}以上）'
        tansho_amount = 100
    elif is_anaba:
        ...
        verdict = f'△  ◎{honmei_name} 単勝 様子見（エッジ{edge_threshold_str}未満）'
        tansho_amount = 0
    ...
    # 馬連◎○プラン（ADI50〜70かつedge_ok）
    honmei_hno = h_main.get('horse_no', '-') if prediction['◎'] else '-'
    umaren = build_umaren_plan(prediction, horse_map, chaos['score'], tansho_amount, edge_ok)
    if umaren['eligible'] and umaren['candidates']:
        print()
        print('  【馬連◎○プラン】')
        print(f"    単勝  ◎馬番{honmei_hno} {honmei_name}  {tansho_amount}円")
        for _, cname, chno in umaren['candidates']:
            print(f"    馬連  ◎馬番{honmei_hno}−○馬番{chno} {cname}  {umaren['amount_per']}円")
        total = tansho_amount + umaren['amount_per'] * len(umaren['candidates'])
        print(f"    合計  {total}円")
    elif umaren['note']:
