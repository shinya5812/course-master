...
[Phase 3 Step 1 変更: 2026-03-19]
  ...
  残り9軸の np.mean() で最終スコアを算出（軸間の重みは引き続き均等）
...
class CourseMASTERv73:
    ...
    def score_horse(self, horse_row, team_type='I'):
        ...
        # [Phase 3 Step 2 Revert] BF重み2倍はバックテストで的中率-1.07%の悪化を確認
        # → 全条件合計で改善47件 vs 悪化70件（悪化が1.5倍多い）
        # 原因: BF軸は全コース総合勝率であり特定コース適性との相関が薄い
        # 対応: 9軸単純平均（np.mean）に戻す
        return float(np.mean(list(axes.values())))
